...
include_once(dirname(__FILE__)."/../curl_helper.php");
include_once(dirname(__FILE__)."/../filters/masks.php");
...
class MyAnimeList{
  ...
  public function login($user, $pass, $api_key, $check_login=true){
    ...
    if ($check_login) {
      $this->_req->request($this->mal_account,$user,$pass);
  ...
  public function search($name, $filter=false){
     // try to found result
     if (defined('DEVDEBUG')) echo "Try raw search - $name<br/>";
